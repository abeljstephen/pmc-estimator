...
var BUILD_INFO = {
  ...
  tag: 'saco-v1.9.28', // ADDED: version bump for CI/KL/sensitivity changes
  ...
  try {
    ...
    if (optimize && probeLevel > 0) {
      ...
      if (!optRes || optRes.error) {
        ...
        // NEW: Sensitivity change calculation (Δprob)
        const baseProbSameTarget = Number.isFinite(targetValue)
          ? coercePercent01(interpolateCdf(cdf, targetValue).value)
          : 0.5;
        const sensitivityChange = Number.isFinite(adjProb) && Number.isFinite(baseProbSameTarget)
          ? adjProb - baseProbSameTarget
          : 0;

        // NEW: Fallback points if optimized points are missing (revert case)
        ...
        if (!Number.isFinite(adjProb)) {
          optimizeBlock = {
            ...
            metrics: { sensitivityChange: sensitivityChange } // NEW: attach even on error
          };
        } else {
          ...
          optimizeBlock = {
            ...
            metrics: { sensitivityChange: sensitivityChange }, // NEW: always attach
            explain,
