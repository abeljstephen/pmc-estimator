 ...
 * @returns {Array} Array of task objects with task name, PERT estimates, and selectedForPlot flag.
 */
function getAllTasks() {
  try {
    ...
    const dataRange = firstSheet.getRange(2, 1, lastRow - 1, Math.max(5, firstSheet.getLastColumn())); // Include SelectedForPlot column
    const data = dataRange.getValues();
    const tasks = data.map(row => {
      const [name, bestCase, mostLikely, worstCase, selectedForPlot] = row;
      ...
      return {
        ...
        selectedForPlot: !!selectedForPlot // Convert to boolean
      };
 ...
 * @returns {Object} Result object with status, message, and errors array for web app context.
 */
function addPertColumns(isWebAppContext = false) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheetName = "Estimate Calculations";
  let sheet = ss.getSheetByName(sheetName);
  let result = { status: 'success', message: '', errors: [] }; // Added errors array
  ...
  const firstSheet = ss.getSheets()[0];
  if (firstSheet.getLastColumn() < 4) {
    Logger.log('Error: First sheet must have fewer than 4 columns');
    sheet.getRange(3, 1).setValue('Error: First sheet must have at least 4 columns (Name, Best Case, Most Likely, Worst Case).');
  ...
  if (lastRow < 2) {
    Logger.log('Error: No data found in the first sheet');
    sheet.getRange(3, 1).setValue('Error: No data found in the first sheet.');
    throw new Error("No data found in the first sheet.");
  }
...
function getProperties(sheetId, sheetName, rowIndex) {
  try {
    ...
    if (typeof bestCase !== 'number' || !isFinite(bestCase) || 
      ...
      throw new Error(`Invalid numeric inputs at row ${rowIndex}: bestCase=${bestCase}, mostLikely=${mostLikely}, worstCase=${bestCase}`);
    }
    if (bestCase > mostLikely || mostLikely > worstCase) {
...
function doGet(e) {
  ...
  try {
    if (e?.parameter?.sheetId) {
      Logger.log('Serving Plot.html with sheetId: ' + e.parameter.sheetId);
 ...
 * @returns {Object} Object containing sheetUrl, plotUrl, message, and errors.
 */
function createEstimateSheet(tasks) {
  Logger.log('createEstimateSheet called with tasks: ' + JSON.stringify(tasks));
  try {
    const timestamp = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyy-MM-dd HH:mm');
    const spreadsheet = SpreadsheetApp.create('Project Estimates ' + timestamp);
    const sheet = spreadsheet.getSheets()[0];
    sheet.setName('Estimation Data');
    const headers = ['Name', 'best_case', 'most_likely', 'worst_case', 'SelectedForPlot'];
    sheet.getRange(1, 1, 1, headers.length).setValues([headers]).setFontWeight('bold');
    const data = tasks.map(task => [
      ...
      task.mostLikely,
      task.worstCase,
      task.selectedForPlot
    ]);
    sheet.getRange(2, 1, data.length, headers.length).setValues(data);
    const calcSheet = spreadsheet.insertSheet('Estimate Calculations');
    const pertResult = addPertColumns(true);
    ...
    Logger.log('Plot URL: ' + plotUrl);
    return {
      sheetUrl: sheetUrl,
      plotUrl: plotUrl,
      message: pertResult.message,
      errors: pertResult.errors || [],
      status: pertResult.status
    };
  } catch (error) {
 ...
 * @param {Object} e - HTTP event with parameters.
 * @returns {TextOutput} JSON containing sheetUrl, plotUrl, message, and errors.
 */
function doPost(e) {
  try {
    ...
    tasks.forEach((task, i) => {
      if (!task.taskName || 
          ...
          typeof task.mostLikely !== 'number' || !isFinite(task.mostLikely) ||
          typeof task.worstCase !== 'number' || !isFinite(worstCase)) {
      ...
      if (task.bestCase > task.mostLikely || task.mostLikely > task.worstCase) {
        throw new Error("Invalid estimate order at index " + i + ": " + JSON.stringify(task));
      }
      if (task.bestCase === task.mostLikely || task.mostLikely === task.worstCase) {
        throw new Error("Estimates too close at index " + i + ": " + JSON.stringify(task));
      }
      const range = task.worstCase - task.bestCase;
      const minRange = task.mostLikely * 0.001;
      if (range < minRange) {
        throw new Error(`Estimate range too small (${range} < ${minRange}) at index ${i}: ` + JSON.stringify(task));
      }
      if (task.bestCase === task.mostLikely && task.mostLikely === task.worstCase) {
        throw new Error("Zero variance at index " + i + ": " + JSON.stringify(task));
      }
    });
    ...
    tasks.forEach((task, i) => {
      sheet.getRange(i + 2, 1, 1, 5).setValues([
        [task.taskName, task.bestCase, task.mostLikely, task.worstCase, task.selectedForPlot]
      ]);
    });
    ...
    const result = {
      sheetUrl: spreadsheet.getUrl() + '#gid=' + calcSheet.getSheetId(),
      plotUrl: plotUrl,
      message: pertResult.message,
      errors: pertResult.errors || [],
      status: pertResult.status
