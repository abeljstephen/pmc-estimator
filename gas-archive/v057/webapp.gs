...
function handleCallApi(body) {
  ...
  // 7. Build chart URLs and report link
...
function buildDistributionChart(res, task) {
  var basePdf = res.baselinePdf  || res.basePdf  || [];
  var adjPdf  = res.adjustedPdf || res.adjPdf   || [];
  if (!basePdf.length && !adjPdf.length) return null;

  var labels   = (basePdf.length ? basePdf : adjPdf).map(function(p){ return Math.round(p.x); });
  var baseData = basePdf.map(function(p){ return p.y != null ? p.y.toFixed(5) : 0; });
  var adjData  = adjPdf.map(function(p){ return p.y != null ? p.y.toFixed(5) : 0; });
