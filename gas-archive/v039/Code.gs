...
function personas(e, context) {
  ...
  try {
    ...
    if (e && e.parameter) {
      ...
      config.sheetName = SpreadsheetApp.getActiveSheet()?.getName() || ESTIMATION_DATA_SHEET_NAME;
      if (context === 'showPlot') {
...
function showPlot() {
  ...
  html.setContent(
    ...
    (CONFIG.SHOW_EXECUTING_USER ? 'var executingUser = ' + JSON.stringify(Session.getEffectiveUser().getEmail()) + ';' : '') +
  ...
  try {
    ...
    if (e?.parameter?.sheetId) {
      ...
      try {
        ...
        Logger.log('Attempting to open spreadsheet: ' + config.sheetId);
        const ss = SpreadsheetApp.openById(config.sheetId);
        Logger.log('Spreadsheet opened successfully');
        const sheet = ss.getSheetByName(config.sheetName || ESTIMATION_DATA_SHEET_NAME);
        if (!sheet) {
          Logger.log('Error: Sheet not found: ' + config.sheetName);
          throw new Error('Sheet not found: ' + config.sheetName);
        }
        Logger.log('Sheet found: ' + config.sheetName);
