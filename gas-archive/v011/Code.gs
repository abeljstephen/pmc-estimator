...
/**
 * Show the HTML form when user opens the Web App, or auto-create the Sheet if ?data= is provided.
 */
function doGet(e) {
  console.log("doGet() STARTED");

  if (e && e.parameter) {
    console.log("doGet() parameters: " + JSON.stringify(e.parameter));

    if (e.parameter.sheetId) {
      console.log("Serving Plot.html for sheetId: " + e.parameter.sheetId);
      return HtmlService.createHtmlOutput("PLOT HTML");
    }

    if (e.parameter.data) {
      console.log("Creating sheet from data param");
      return ContentService.createTextOutput(
        JSON.stringify({ result: "SUCCESS: data param received" })
      ).setMimeType(ContentService.MimeType.JSON);
    }
  }

  console.log("Serving default submit.html");
  return HtmlService.createHtmlOutput("SUBMIT HTML");
}
...
function createEstimateSheet(tasks) {
  ...
  // Create new Spreadsheet in the user's Drive
  const spreadsheet = SpreadsheetApp.create("Project Estimates");
  const sheet = spreadsheet.getActiveSheet();
  sheet.clear();
  sheet.getRange("A1:D1").setValues([
    ["Task Name", "Best Case", "Most Likely", "Worst Case"]
  ]);

  tasks.forEach((task, i) => {
    sheet.getRange(i + 2, 1, 1, 4).setValues([
      [task.taskName, task.bestCase, task.mostLikely, task.worstCase]
    ]);
  });

  addPERTColumns(); // Your existing function to add calculations
 ...
 * Expects: { "tasks": [ { taskName, bestCase, mostLikely, worstCase } ] }
 * Returns: JSON { sheetUrl, plotUrl }
 */
function doGet(e) {
