  ...
  try {
    if (e && e.parameter && e.parameter.sheetId) {
      ...
      return HtmlService.createHtmlOutputFromFile('Plot')
        ...
        .append('<script>var sheetId = "' + Utilities.base64Encode(e.parameter.sheetId) + '";</script>');
    }
    if (e && e.parameter && e.parameter.data) {
      Logger.log('Serving submit.html with data: ' + e.parameter.data);
    ...
    Logger.log('Serving submit.html (no parameters)');
...
/**
 * Processes a batch of estimates from submit.html and creates a spreadsheet.
 * @param {Array} estimates - Array of estimate objects with bestCase, mostLikely, worstCase.
 * @returns {Object} Object containing sheetUrl and plotUrl.
 */
function estimateAndSave(estimates) {
  try {
    if (!Array.isArray(estimates) || estimates.length === 0) {
      throw new Error("Invalid or empty estimates array.");
    }

    // Validate estimates and add taskName
    const tasks = estimates.map((estimate, index) => {
      if (typeof estimate.bestCase !== 'number' || !isFinite(estimate.bestCase) ||
          typeof estimate.mostLikely !== 'number' || !isFinite(estimate.mostLikely) ||
          typeof estimate.worstCase !== 'number' || !isFinite(estimate.worstCase)) {
        throw new Error(`Invalid estimate data at index ${index}: ${JSON.stringify(estimate)}`);
      }
      if (estimate.bestCase > estimate.mostLikely || estimate.mostLikely > estimate.worstCase) {
        throw new Error(`Invalid estimate order at index ${index}: ${JSON.stringify(estimate)}`);
      }
      if (estimate.bestCase === estimate.mostLikely || estimate.mostLikely === estimate.worstCase) {
        throw new Error(`Estimates too close at index ${index}: ${JSON.stringify(estimate)}`);
      }
      const range = estimate.worstCase - estimate.bestCase;
      const minRange = estimate.mostLikely * 0.001;
      if (range < minRange) {
        throw new Error(`Estimate range too small (${range} < ${minRange}) at index ${index}: ${JSON.stringify(estimate)}`);
      }
      if (estimate.bestCase === estimate.mostLikely && estimate.mostLikely === estimate.worstCase) {
        throw new Error(`Zero variance at index ${index}: ${JSON.stringify(estimate)}`);
      }
      return {
        taskName: `Task ${index + 1}`, // Auto-generate task name
        bestCase: estimate.bestCase,
        mostLikely: estimate.mostLikely,
        worstCase: estimate.worstCase,
        selectedForPlot: index === 0 // Select first task for plotting
      };
    });

    // Reuse createEstimateSheet to process tasks
    const result = createEstimateSheet(tasks);
    return { ...result, results: tasks }; // Return sheetUrl, plotUrl, and task count
  } catch (error) {
    Logger.log('Error in estimateAndSave: ' + error.message);
    throw error;
  }
}
