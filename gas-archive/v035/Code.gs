  ...
  try {
    ...
    if (e && e.parameter) {
      ...
      if (e.parameter.sheetId) {
        ...
        config.sheetName = 'Estimation Data'; // Web app sheets use Estimation Data
        // Default task set in createEstimateSheet or doPost
      } else {
        // submit.html context, no sheetId yet
      ...
      config.sheetName = SpreadsheetApp.getActiveSheet().getName();
      if (context === 'showPlot') {
        ...
        // Get tasks to set defaultTask
        const tasks = getAllTasks(config.sheetId);
        if (tasks.length > 0) {
          const sheet = ss.getSheetByName(config.sheetName);
          if (sheet && config.rowIndex <= sheet.getLastRow()) {
            const taskData = sheet.getRange(config.rowIndex, 1, 1, 4).getValues()[0];
            config.defaultTask = taskData[0] || 'Unnamed Task';
          } else {
            config.defaultTask = tasks[0].task;
            config.rowIndex = 2;
          }
        }
      }
...
function getAllTasks(sheetId) {
  try {
    ...
    const dataRange = firstSheet.getRange(2, 1, lastRow - 1, Math.max(5, firstSheet.getLastColumn()));
    ...
    const tasks = data.map((row, index) => {
      ...
      if (typeof bestCase !== 'number' || !isFinite(bestCase) || 
        ...
        Logger.log(`Skipping row ${index + 2} with non-numeric or non-finite estimates: ${JSON.stringify(row)}`);
        return null;
      }
      if (bestCase > mostLikely || mostLikely > worstCase) {
        Logger.log(`Skipping row ${index + 2} with invalid estimate order: ${JSON.stringify(row)}`);
        return null;
      }
      return {
    ...
    // Determine default task index (web: selectedForPlot, standalone: handled by personas)
    let defaultTaskIndex = 0;
    const selectedTask = tasks.find(task => task.selectedForPlot);
    if (selectedTask) {
      defaultTaskIndex = tasks.indexOf(selectedTask);
    }
  ...
  try {
    ...
    tasks.forEach((task, i) => {
      ...
      if (task.selectedForPlot) {
        if (selectedTask) {
          throw new Error('Multiple tasks selected for plotting at index ' + i);
        }
    ...
    const config = personas(null, 'createEstimateSheet');
    config.sheetId = spreadsheet.getId();
    config.sheetName = 'Estimation Data';
    config.defaultTask = selectedTask;
    
    const calcSheet = spreadsheet.insertSheet('Estimate Calculations');
    const pertResult = addPertColumns(config);
    
    const sheetUrl = spreadsheet.getUrl() + '#gid=' + calcSheet.getSheetId();
    const plotUrl = ScriptApp.getService().getUrl() + '?sheetId=' + config.sheetId;
