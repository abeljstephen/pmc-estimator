...
function addPertColumns(isWebAppContext = false) {
  ...
  const tasks = data.map((row, index) => {
    ...
    if (typeof bestCase !== 'number' || !isFinite(bestCase) || 
      ...
      result.errors.push(error); // Add to result.errors
  ...
  if (validTasks.length > 0) {
    try {
      ...
      result.errors.push(`API call failed: ${error.message}`); // Add to result.errors
  ...
  if (errorMessages.length > 0) {
    result.message = errorMessages.join('; ');
    result.status = 'error'; // Mark status as error
  } else {
...
function createEstimateSheet(tasks) {
  ...
  try {
    ...
    return {
      ...
      errors: pertResult.errors || [], // Include errors array
