// File: optimization/optimizer_final.gs
// SACO Geometry Optimizer v1.9.34 — Latin Hypercube + Explicit SACO Geometry Components
...
// FIXED: pct() now ALWAYS returns number (0–1), never string — eliminates finalProb string bug
// REMOVED: Obsolete "Fallback to baseline points applied" log message — new reshaped points always attached after revert
...
// FIXED: v1.9.34 — pct() hardened to number-only return; removed misleading fallback log; added startup version debug
...
console.log('DEBUG: optimizer_final.gs STARTUP - pct test type:', typeof pct(0.5));
console.log('DEBUG: optimizer_final.gs LOADED - pct test type:', typeof pct(0.5));
...
console.log('OPTIMIZER v1.9.34 — SACO Geometry + Latin Hypercube');
...
function pct(n) {
  return clamp01(safeNumber(n));  // returns number 0–1 - FORCE FIXED - no string formatting
}
function lerp(a, b, t) {
  return (1 - t) * a + t * b;
}
...
function step7_output(state) {
  console.log('*** STEP7 v1.9.34 - CRASH-PROOF VERSION - ALL toFixed GUARDED ***');
