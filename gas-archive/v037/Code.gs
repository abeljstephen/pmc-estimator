...
function callEstimatorAPI(tasks) {
  ...
  try {
    if (!Array.isArray(tasks) || tasks.length === 0) {
      logMinimal('Error: tasks must be a non-empty array');
    ...
    tasks.forEach((task, i) => {
      if (!task.task || 
        ...
        logMinimal('Invalid task data at index ' + i + ': ' + JSON.stringify(task));
    ...
    if (!keyJsonString) {
      logMinimal('Error: SERVICE_ACCOUNT_KEY not found in Script Properties');
    ...
    try {
      ...
      logMinimal('Error parsing SERVICE_ACCOUNT_KEY: ' + e.message);
    ...
    try {
      ...
      logMinimal('Generated JWT: [Redacted]');
      ...
      if (Date.now() - startTime > 30000) {
        logMinimal('Error: Token request timed out');
        throw new Error('Token request timed out');
      }
      logMinimal('Token endpoint response code: ' + response.getResponseCode());
      logMinimal('Token endpoint response: [Redacted]');
      if (response.getResponseCode() !== 200) {
        throw new Error('Failed to obtain token: ' + response.getContentText());
      }
      const tokenData = JSON.parse(response.getContentText());
      token = tokenData.id_token;
      logMinimal('Generated Token: [Redacted]');
    } catch (e) {
      logMinimal('Token generation failed: ' + e.message);
    ...
    logMinimal('Calling API with ' + tasks.length + ' tasks');
    logMinimal('Payload: ' + JSON.stringify(tasks));
    const response = UrlFetchApp.fetch(url, options);
    if (Date.now() - startTime > 30000) {
      logMinimal('Error: API request timed out');
      throw new Error('API request timed out');
    }
    logMinimal('API response code: ' + response.getResponseCode());
    logMinimal('API response content: ' + response.getContentText());
    if (response.getResponseCode() !== 200) {
      logMinimal('API request failed: ' + response.getContentText());
      throw new Error('API request failed: ' + response.getContentText());
    }
    const responseData = JSON.parse(response.getContentText());
    logMinimal('API response data received with ' + (responseData.results ? responseData.results.length : 0) + ' results');
    return responseData;
  } catch (error) {
    logMinimal('Error in callEstimatorAPI: ' + error.message + '\nStack: ' + error.stack);
    throw error;
  }
}
...
function doGet(e) {
  ...
  try {
    ...
    try {
      template.tasks = e?.parameter?.tasks ? decodeURIComponent(e.parameter.tasks.replace(/\+/g, ' ')) : '';
      Logger.log('Decoded tasks for submit.html: ' + template.tasks);
    } catch (decodeError) {
      Logger.log('Error decoding tasks parameter: ' + decodeError.message);
      template.tasks = '';
    }
    if (Date.now() - startTime > 5000) { // 5-second timeout
...
function createEstimateSheet(tasks) {
  logMinimal('createEstimateSheet called with tasks: ' + JSON.stringify(tasks));
  try {
    // Validate tasks
    ...
    logMinimal('Error in createEstimateSheet: ' + error.message);
