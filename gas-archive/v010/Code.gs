...
/**
 * Handles POST requests from the GPT to process task data.
 * Accepts JSON task data (manual or CSV/Excel), writes to "Input" sheet, runs addPERTColumns,
 * and returns URLs for Plot.html and the Google Sheet.
 * @param {Object} e - Event object with POST data.
 * @returns {ContentService} JSON response with webAppUrl and sheetUrl.
 */
function doPost(e) {
  try {
    // Parse JSON task data from GPT (e.g., [{"taskName": "Task A", "bestCase": 2, "mostLikely": 4, "worstCase": 6}])
    ...
    data.forEach((task, index) => {
      if (!task.taskName || isNaN(task.bestCase) || isNaN(task.mostLikely) || isNaN(task.worstCase) ||
          task.bestCase > task.mostLikely || task.mostLikely > task.worstCase) {
        throw new Error(`Invalid task at index ${index}: Ensure taskName is non-empty and bestCase <= mostLikely <= worstCase.`);
      }
    });

    // Access or create Input sheet
    var spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
    var sheet = spreadsheet.getSheetByName('Input') || spreadsheet.insertSheet('Input');
    sheet.clear(); // Clear existing data to avoid conflicts
    
    // Write headers and task data
    sheet.getRange('A1:D1').setValues([['Task Name', 'Best Case', 'Most Likely', 'Worst Case']]);
    data.forEach((task, index) => {
      sheet.getRange(index + 2, 1, 1, 4).setValues([
        [task.taskName, parseFloat(task.bestCase), parseFloat(task.mostLikely), parseFloat(task.worstCase)]
      ]);
    });

    // Run addPERTColumns to generate Estimate Calculations sheet
    addPERTColumns();

    // Generate URLs for Plot.html and Google Sheet
    var sheetId = spreadsheet.getId();
    var webAppUrl = ScriptApp.getService().getUrl() + '?sheetId=' + sheetId;
...
/**
 * Handles GET requests to serve Plot.html or fetch metrics for the GPT.
 * Serves Plot.html with sheetId for visualizations or returns Estimate Calculations data.
 * @param {Object} e - Event object with query parameters (sheetId, action).
 * @returns {HtmlService|ContentService} HTML for Plot.html or JSON with metrics.
 */
function doGet(e) {
  try {
    // Check for metrics request (for follow-up questions)
    if (e.parameter.action === 'getMetrics') {
      var sheetId = e.parameter.sheetId;
      if (!sheetId) throw new Error('sheetId is required for getMetrics.');
      var spreadsheet = SpreadsheetApp.openById(sheetId);
      var sheet = spreadsheet.getSheetByName('Estimate Calculations');
      if (!sheet) throw new Error('Estimate Calculations sheet not found.');
      var data = sheet.getDataRange().getValues();
      return ContentService.createTextOutput(JSON.stringify(data))
        .setMimeType(ContentService.MimeType.JSON);
    }

    // Serve Plot.html for visualization
    var sheetId = e.parameter.sheetId;
    if (!sheetId) throw new Error('sheetId is required for Plot.html.');
    var html = HtmlService.createHtmlOutputFromFile('Plot')
      .setTitle('Project Estimation Dashboard')
      .addMetaTag('viewport', 'width=device-width, initial-scale=1');
    html.append('<script>var sheetId = "' + sheetId + '";</script>');
    return html;
  } catch (error) {
