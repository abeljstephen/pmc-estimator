...
// SACO Geometry Optimizer v1.9.27 — Latin Hypercube + Explicit SACO Geometry Components
...
var PER_SLIDER_BOUNDS = W_MEAN.map(w => ({ lo: w < 0 ? 0.15 : 0, hi: w > 0 ? 0.7 : 1.0 }));
console.log('OPTIMIZER v1.9.28 — SACO Geometry + Latin Hypercube');
...
var SACO_GEOMETRY = {
  baseline: function(params) {
    console.log('SACO_GEOMETRY.baseline placeholder called:', params);
    return { pdfPoints: [], cdfPoints: [] };
  },
  lhsSample: function(n, dims, seed = Date.now()) {
    console.log('lhsSample placeholder: n=', n, 'dims=', dims);
  ...
  computeMoments: function(sliders100, scale, cv) {
    console.log('computeMoments placeholder:', sliders100);
    return { moments: [0, 0] };
  },
  ...
  klDivergence: function(params) {
    console.log('klDivergence placeholder:', params);
...
function applyReshapeRules(x, mode, probeLevel = 1, seedBest = null, state = {}) {
  ...
  if (mode === 'adaptive' && seedBest && probeLevel > 1) {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      // UPDATED: Relaxed threshold from 0.05 → 0.08 to reduce aggressive reversion
      // while still enforcing strong anchoring to seed geometry (SACO-preserving)
      if (div > 0.08) {
        x[i] = lerp(x[i], seedBest[i], 0.8);
        console.log(`ANCHOR ADJUST: slider ${i} dev=${div.toFixed(3)} > 0.08 → lerped to seed`);
      }
    }
...
function cobyla(objective, initial, o, m, p, tau, basePdf, baseCdf, opts = {}, adaptive, probeLevel, seedBest, p0) {
  ...
  while (iter++ < cfg.maxIter && rho > cfg.rhoFinal) {
    ...
    if (adaptive && iter % 10 === 0 && seedBest) {
      ...
      if (maxDiv > 0.08) { // UPDATED: aligned with applyReshapeRules threshold
...
/* ---------------------------- Step helpers ----------------------------- */
function safeProbeLevel(v) { const n = Math.floor(Number(v)); return Number.isFinite(n) ? Math.max(1, Math.min(7, n)) : 5; }
...
function step1_baseline(state) {
  ...
  // FIXED: Keep p0 as number (0–1), not string from pct()
  const p0Raw = interpolateCdf(base.cdfPoints, tau).value || 0;
  const p0 = p0Raw; // number for math (fixes p0.toFixed error later)
  const mu = (o + 4 * m + p) / 6;
...
/* Step 3 — Warm Start (first 3 dimensions) */
...
/* Step 4 — Search (evaluate all LHS points) */
...
/* Step 5 — Refine (COBYLA local search) */
...
function step7_output(state) {
  ...
  if (state.adaptive) {
    ...
    for (let i = 0; i < 7; i++) {
      const div = Math.abs(x[i] - seedBest[i]) / Math.max(seedBest[i] || 0.01, 1e-6);
      // UPDATED: threshold now 0.08 (was 0.05) — tunable SACO anchor relaxation
      if (div > 0.08) {
    ...
    if (reverted) {
      ...
      // ADDED: log reverted sliders for debugging
  ...
  if (refit) {
    try {
      const betaPts = generateBetaPoints({ optimistic: o, mostLikely: m, pessimistic: p, numSamples: baseline.pdfPoints.length || 200, alpha: refit.alpha, beta: refit.beta });
      if (betaPts.pdfPoints?.length > 1) {
        reshapedPdf = betaPts.pdfPoints;
        reshapedCdf = betaPts.cdfPoints;
      }
    } catch (e) {
      console.warn('Step7: generateBetaPoints failed', e.message);
    }
  }
