 ...
 * 
 * Usage: Import in main.js and optimizer.js: const { generateMonteCarloBaseline, computeBaselineMetrics } = require('./utils');
 */
 ...
 * Output: CI object (lower/upper bounds) and baseProb (P(X <= target)).
...
function computeBaselineMetrics(baselineData, pertMean, targetValue, confidenceLevel = 0.95) {
  ...
  const ci = {
    lower: samples[lowIdx],
    upper: samples[highIdx],
    confidenceLevel  // e.g., 0.95 for 95% CI
  };
  ...
  return {
    ci: {
      monteCarloSmoothed: ci  // Nested for schema compatibility
    },
    baseProb  // Undefined if no target
  };
}

//module.exports = { generateMonteCarloBaseline, computeBaselineMetrics };
