...
// Section: Task Retrieval for Plot.html Dropdown
// Purpose: Fetches all tasks from the first sheet of the active Google Spreadsheet, 
// validating that the sheet has at least 4 columns (Name, Best Case, Most Likely, Worst Case) 
// and data starting from row 2. Ensures estimates are numeric and in valid order 
// (bestCase ≤ mostLikely ≤ worstCase). Returns an array of task objects with task name 
// and PERT estimates (optimistic, most likely, pessimistic) for use in Plot.html’s 
// "Select Task" dropdown, enabling dynamic task selection for probability simulations.
// Called via google.script.run from Plot.html. Logs errors for debugging and skips 
// invalid rows (non-numeric estimates or incorrect order).
